...
class PrimsLib {
    ...
    function upload_file($file, $name, $format, $size){
        $ci =& get_instance();
        if ($name != '') {
            $config['upload_path'] = './assets/file_santri/' . $file;
            ...
            if (!$ci->upload->do_upload($file))
            {
                $error = array('error' => $ci->upload->display_errors(),
                                'tb_santri' => $ci->m_data_santri->getAll('tb_santri')->result(),
                                'custom' => $ci->lang->line('Pengunggahan file' . $file . 'Gagal!')
                );
                echo $ci->load->view('admin_template/header', array(), TRUE);
                echo $ci->load->view('admin_template/mainmenu', array(), TRUE);
                echo $ci->load->view('admin/v_santri', $error, TRUE);
                echo $ci->load->view('admin_template/footer', array(), TRUE);
                exit;
            }
            else
            {
